...
# ====================================================================
# HÀM KẾT NỐI CSDL (Thay thế cho connect_db())
# SỬ DỤNG PYODBC VÀ SQL SERVER
# ====================================================================
...
def get_connection():
    ...
    try:
        ...
        sqlstate = ex.args[0]
        # Hiển thị lỗi kết nối chi tiết (Ví dụ: Server không bật, sai tên CSDL/Server)
        messagebox.showerror("Lỗi Kết Nối CSDL", f"Không thể kết nối đến SQL Server:\n{sqlstate}")
        return None

# Đặt lại tên hàm kết nối mà các hàm khác sử dụng
...
def load_all_data():
    ...
    try:
        # Sử dụng cú pháp SQL chuẩn (không cần thay đổi)
        ...
        for row in rows:
            ...
            # Định dạng lại giá và số lượng để hiển thị đẹp hơn
...
def them_sp():
    ...
    try:
        # Kiểm tra masp trùng (Sử dụng '?' cho pyodbc thay vì '%s')
        cur.execute("SELECT masp FROM SanPham WHERE masp = ?", (masp,))
        if cur.fetchone():
            messagebox.showerror("Lỗi Thêm", "Mã Sản phẩm đã tồn tại.")
            return
            
        sql = "INSERT INTO SanPham (masp, tensp, giasp, soluongton) VALUES (?, ?, ?, ?)"
        cur.execute(sql, (masp, tensp, giasp, soluongton))
        conn.commit()
        messagebox.showinfo("Thành công", "Đã thêm Sản phẩm mới.")
        load_all_data()
        clear_input_sp()
    except pyodbc.Error as err: # Thay mysql.connector.Error bằng pyodbc.Error
...
def sua_sp_select(event):
    ...
    # values[2] là giá đã được định dạng (ví dụ: 10,000)
    # Cần loại bỏ dấu phân cách (,) để nhập lại vào Entry
...
def xoa_sp():
    ...
    # Lấy masp từ cột đầu tiên của Treeview item
    ...
    try:
        ...
        # Có thể gặp lỗi nếu Sản phẩm đã được sử dụng trong đơn hàng (khóa ngoại)
...
def update_cthd_tree():
    ...
    for i, item in enumerate(cthd_temp):
        ...
        # Hiển thị giá và thành tiền có định dạng
...
def them_sp_vao_don():
    ...
    try:
        ...
        # Ép kiểu masp về kiểu dữ liệu ban đầu (ví dụ: int nếu masp là số)
        try:
            masp = int(selected_sp_str.split(' - ')[0])
        except ValueError:
            masp = selected_sp_str.split(' - ')[0] # Giữ nguyên string nếu masp không phải số
...
def lap_don_hang():
    ...
    try:
        ...
        # 1. Thêm vào bảng DonHang (Sử dụng '?' cho pyodbc)
        sql_dh = "INSERT INTO DonHang (ngaydat, tongtien) VALUES (?, ?)"
        cur.execute(sql_dh, (ngaydat, tong_tien))
        
        # Lấy Mã Đơn Hàng mới được tạo (SQL Server/pyodbc)
        # Sử dụng SELECT SCOPE_IDENTITY() để lấy ID mới nhất trong cùng một scope
...
def xoa_sp_trong_don():
    ...
    try:
        # Lấy masp (cột 0)
        masp_can_xoa = tree_cthd.item(selected_item, 'values')[0] 
        # Cần đảm bảo kiểu dữ liệu phù hợp (int hoặc string)
        try:
            masp_can_xoa = int(masp_can_xoa)
        except ValueError:
            pass
    except:
    ...
    global cthd_temp
    # Lọc danh sách, giữ lại các item có mã sản phẩm khác với mã cần xóa
...
def on_sp_select(event):
    ...
    try:
        # Lấy masp (phần trước dấu ' - ')
        masp = selected_sp_str.split(' - ')[0] 
        try:
            masp = int(masp) # Cố gắng chuyển sang int nếu là số
        except ValueError:
            pass # Giữ nguyên string nếu không phải số
